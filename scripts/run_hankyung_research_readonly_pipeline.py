...
DEFAULT_SUMMARY_OUTPUT = DATA_DIR / "hankyung_research_summary_latest.md"
DEFAULT_FACTOR_OUTPUT = DATA_DIR / "hankyung_research_factor_impact_latest.md"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Hankyung consensus research pipeline without orders."
    )
    parser.add_argument("--url", default=HANKYUNG_CONSENSUS_URL)
    parser.add_argument("--source", default="hankyung_consensus")
    parser.add_argument("--broker", default="한경 컨센서스")
...
def build_pipeline_steps(args: argparse.Namespace) -> list[PipelineStep]:
    ...
    date_args = ["--start-date", args.start_date.isoformat()]
    if args.end_date is not None:
        date_args.extend(["--end-date", args.end_date.isoformat()])
    return [
        PipelineStep(
            "sync",
            [
